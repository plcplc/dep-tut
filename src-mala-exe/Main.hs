...
instance (Serialise (f (Fix f))) => Serialise (Fix f)
...
instance Semigroup (Vertically BoundingBox) where
  Vertically bb1 <> Vertically bb2 =
    Vertically
      BoundingBox
        { bbWidth = max (bbWidth bb1) (bbWidth bb2),
...
instance Semigroup (Horizontally BoundingBox) where
  Horizontally bb1 <> Horizontally bb2 =
    Horizontally
      BoundingBox
        { bbHeight = max (bbHeight bb1) (bbHeight bb2),
...
instance (Serialise (f a), Serialise (g a)) => Serialise ((f :+: g) a)
...
bbGrout' _c bbDyn ui = do
  ...
  grout (fixed $ bbHeight <$> bbDyn) $ row $ grout (fixed $ bbWidth <$> bbDyn) ui
...
bbGrout _c bbDyn ui = do
  ...
  --
  -- assumes already in 'row' axis.
  grout (fixed $ bbWidth <$> bbDyn) $ col $ grout (fixed $ bbHeight <$> bbDyn) $ row ui
...
bbGrout2 _c currentOrientation bbDyn ui =
  grout (fixed (boxEdge <$> currentOrientation <*> bbDyn)) $
    axis (flipOrientation <$> currentOrientation) flex $
      grout (fixed (boxEdge . flipOrientation <$> currentOrientation <*> bbDyn)) $
        row ui
  where
    -- \$ fill (pure c) >>
  ...
  nodeEditor (Fix alt) = tneGrout $ case alt of
    ...
    where
      ...
      termEditor title (SOP (Z args)) = mdo
        ...
        (argValuesDyn, argWidthsDyn) <-
          axis selectedOrientationDyn flex $
            runDynamicWriterT $
              unComp $
                hcfor (Proxy @TermExpressionEditor) args (\(I arg) -> Comp (argEditor arg))
...
instance (Serialise t) => Serialise (SurfaceTerm t)
...
-- | 'Desugaring' here currently means:
--
--  * The removing of holes
--  * Translating 'SurfaceTerm's to 'Term i's (monomorphised as 'SomeTerm')
--  * (^ including named representation to de-bruijn indices)
...
{-# ANN MalaState ("hlint: ignore Use newtype instead of data" :: String) #-}

data MalaState = MalaState
  {msCurrentSurfaceExpression :: Fix (Hole :+: SurfaceTerm)}
  deriving (GHC.Generics.Generic)

instance Serialise MalaState

initialMalaState :: MalaState
initialMalaState = MalaState (Fix (InL Hole))

main :: IO ()
main = do
  exists <- doesFileExist "mala.cbor"
  unless exists $ do
    writeFileSerialise "mala.cbor" initialMalaState

  st :: MalaState <- readFileDeserialise "mala.cbor"

  mainWidget $ widget st
  where
    widget ::
      forall t m.
      ( MonadVtyApp t m,
    ...
    widget st = mdo
      ...
      updateSt <- initManager_ $ mdo
        ...
        grout flex $
          boxTitle (pure singleBoxStyle) "Desugared" $
            grout (fixed 3) $ do
              row $ text (displayTerm <$> current termD)

        termD <- grout flex $ termExpressionEditorMain (msCurrentSurfaceExpression st)

        return saveCurrentEv
